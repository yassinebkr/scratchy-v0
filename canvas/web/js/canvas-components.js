...
_canvasRenderers.stats = function(d) {
  var items = d.items || [];
  var h = d.title ? '<div style="font-size:0.75rem;color:#888;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:10px;">' + esc(d.title) + '</div>' : '';
...
_canvasRenderers.card = function(d) {
  ...
  return '<div style="padding:14px 16px;">' +
    ...
    '</div>' +
    '<div style="font-size:0.82rem;color:#aaa;line-height:1.5;">' + esc(d.text) + '</div>' +
  '</div>';
};
...
_canvasRenderers.alert = function(d) {
  ...
  return '<div style="padding:12px 14px;border-left:3px solid ' + c + ';display:flex;gap:10px;align-items:flex-start;">' +
    ...
    '<div>' +
      ...
      '<div style="font-size:0.78rem;color:#999;line-height:1.4;">' + esc(d.message || d.text || '') + '</div>' +
    '</div>' +
  '</div>';
};
...
_canvasRenderers["chart-bar"] = function(d) {
  ...
  var h = d.title ? '<div style="font-size:0.75rem;color:#888;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:12px;">' + esc(d.title) + '</div>' : '';
  var bars = '';
  for (var i = 0; i < items.length; i++) {
    var it = items[i];
...
_canvasRenderers["chart-line"] = function(d) {
  ...
  var title = d.title ? '<div style="font-size:0.75rem;color:#888;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:8px;">' + esc(d.title) + '</div>' : '';
...
_canvasRenderers.kv = function(d) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    rows += '<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid rgba(255,255,255,0.04);">' +
      '<span style="font-size:0.78rem;color:#888;">' + esc(pair[0]) + '</span>' +
      '<span style="font-size:0.78rem;color:#ddd;font-weight:500;">' + esc(pair[1]) + '</span>' +
    '</div>';
  }
...
_canvasRenderers.code = function(d) {
  ...
  return '<div style="padding:0;">' +
    (lang ? '<div style="padding:6px 14px;font-size:0.68rem;color:#666;border-bottom:1px solid rgba(255,255,255,0.04);text-transform:uppercase;letter-spacing:0.05em;">' + esc(lang) + '</div>' : '') +
...
_canvasRenderers.progress = function(d) {
  ...
  return '<div style="padding:12px 16px;">' +
    ...
    '<div style="height:6px;background:rgba(255,255,255,0.06);border-radius:3px;overflow:hidden;">' +
      '<div style="height:100%;width:' + val + '%;background:linear-gradient(90deg,#7c3aed,#3b82f6);border-radius:3px;transition:width 0.5s ease;"></div>' +
    '</div>' +
  '</div>';
};
...
_canvasRenderers.gauge = function(d) {
  ...
  var svg = '<svg viewBox="0 0 90 55" style="width:90px;height:55px;">' +
    ...
    '<path d="M 9 50 A 36 36 0 0 1 81 50" fill="none" stroke="' + c + '" stroke-width="' + stroke + '" stroke-linecap="round" ' +
      'stroke-dasharray="' + filled + ' ' + circumference + '" style="transition:stroke-dasharray 0.5s ease;" />' +
  '</svg>';
...
_canvasRenderers.buttons = function(d) {
  ...
  for (var i = 0; i < opts.length; i++) {
    html += '<button data-sui-send="' + esc(opts[i]) + '" style="padding:6px 14px;border-radius:6px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.04);color:#ddd;font-size:0.78rem;cursor:pointer;transition:background 0.15s,border-color 0.15s;"' +
...
_canvasRenderers.accordion = function(d) {
  ...
  for (var i = 0; i < sections.length; i++) {
    ...
    html += '<details' + (open ? ' open' : '') + ' style="border-bottom:1px solid rgba(255,255,255,0.04);padding:8px 0;">' +
      ...
      '<div style="font-size:0.78rem;color:#999;padding:6px 0;line-height:1.5;">' + esc(s.text) + '</div>' +
    '</details>';
  }
...
_canvasRenderers.form = function(d) {
  ...
  // Title
  var html = d.title ? '<div style="font-size:0.75rem;color:#888;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:10px;">' + esc(d.title) + '</div>' : '';
  ...
  if (actions.length > 0) {
    ...
    for (var i = 0; i < actions.length; i++) {
      var btnText = actions[i];
      html += '<button type="button" data-sui-form="' + esc(formId) + '" data-sui-send="' + esc(btnText) + '" style="padding:6px 14px; border-radius:6px; border:1px solid rgba(255,255,255,0.1); background:rgba(255,255,255,0.04); color:#ddd; font-size:0.78rem; cursor:pointer; transition:background 0.15s, border-color 0.15s;"' +
        ' onmouseover="this.style.background=\'rgba(124,58,237,0.15)\';this.style.borderColor=\'rgba(124,58,237,0.3)\'"' +
        ' onmouseout="this.style.background=\'rgba(255,255,255,0.04)\';this.style.borderColor=\'rgba(255,255,255,0.1)\'"' +
        '>' + esc(btnText) + '</button>';
    }
...
_canvasRenderers["chart-bar"] = function(d) {
  ...
  for (var i = 0; i < labels.length; i++) {
    ...
    for (var j = 0; j < datasets.length; j++) {
      ...
      bars += '<rect x="' + (gx + j * (barW + 1)) + '" y="' + (padT + chartH - h) + '" width="' + barW + '" height="' + h + '" rx="3" fill="' + c + '" opacity="0.85"><title>' + esc(labels[i]) + ': ' + val + '</title></rect>';
    }
  ...
  return '<div style="padding:14px 12px;">' +
    (d.title ? '<div style="font-size:0.75rem;color:#888;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:8px;">' + esc(d.title) + '</div>' : '') +
...
_canvasRenderers["chart-pie"] = function(d) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    slices += '<path d="M' + cx + ',' + cy + ' L' + x1 + ',' + y1 + ' A' + r + ',' + r + ' 0 ' + large + ',1 ' + x2 + ',' + y2 + ' Z" fill="' + c + '" opacity="0.85"><title>' + esc(items[i].label) + ': ' + items[i].value + '</title></path>';
    angle = a2;
  }
  var legend = items.map(function(it, i) {
    var c = it.color || colors[i % colors.length];
    return '<div style="display:flex;align-items:center;gap:6px;margin:2px 0;"><span style="width:8px;height:8px;border-radius:2px;background:'+c+';flex-shrink:0;"></span><span style="font-size:0.78rem;color:#bbb;">'+esc(it.label)+'</span><span style="font-size:0.72rem;color:#666;margin-left:auto;">'+it.value+'</span></div>';
  }).join('');
  return '<div style="padding:14px 12px;">' +
    (d.title ? '<div style="font-size:0.75rem;color:#888;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:8px;">'+esc(d.title)+'</div>' : '') +
...
_canvasRenderers.streak = function(d) {
  ...
  return '<div style="padding:14px 12px;">' +
    (d.title ? '<div style="font-size:0.75rem;color:#888;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:10px;">'+esc(d.title)+'</div>' : '') +
...
_canvasRenderers.rating = function(d) {
  ...
  return '<div style="padding:14px 12px;text-align:center;">' +
    (d.title ? '<div style="font-size:0.75rem;color:#888;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:6px;">'+esc(d.title)+'</div>' : '') +
    '<div>'+stars+'</div></div>';
};
...
_canvasRenderers.chips = function(d) {
  ...
  var html = chips.map(function(c) {
    ...
    return '<span style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:0.75rem;background:'+color+'22;color:'+color+';border:1px solid '+color+'33;margin:3px;">'+esc(label)+'</span>';
  }).join('');
  return '<div style="padding:14px 12px;">' +
    (d.title ? '<div style="font-size:0.75rem;color:#888;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:8px;">'+esc(d.title)+'</div>' : '') +
...
_canvasRenderers.tabs = function(d) {
  ...
  return '<div>' +
    (d.title ? '<div style="padding:12px 14px 0;font-size:0.75rem;color:#888;text-transform:uppercase;letter-spacing:0.05em;">'+esc(d.title)+'</div>' : '') +
