class CanvasState {
  ...
  /**
   * Apply multiple operations as a batch.
   * Fires individual notifications per op so morphdom can diff smoothly.
   * Only falls back to "reset" if the batch starts with "clear".
   * @param {Array<Object>} ops
   */
  applyBatch(ops) {
    if (!ops || ops.length === 0) return;

    // If batch starts with clear + upserts, suppress clear notification
    // and let upserts create tiles fresh (no flash)
    var startsWithClear = ops[0] && ops[0].op === "clear";
    
    if (startsWithClear && ops.length > 1) {
      // Silently clear state, then apply rest normally
      this.components = {};
      this.version++;
      this._notify("clear", {});
      ...
      // Normal: apply each op individually (morphdom handles smooth updates)
